...
func (c *COSStorage) Put(ctx context.Context, bucket string, key string, reader io.Reader) error {
	// TODO: 实现 COS 文件上传
	return errors.New("COS storage not implemented yet")
}

// GetPresignedURL 获取预签名下载 URL (空实现)
func (c *COSStorage) GetPresignedURL(ctx context.Context, bucket string, key string, expiry time.Duration) (string, error) {
